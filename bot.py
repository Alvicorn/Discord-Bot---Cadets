...
# tasks that are pending to be approved
pendingList = []
...
def displayTaskInfo_name(taskName, title):
    ...
    task = ("Task Name:\t{6}{6}{6}{0}\n" + 
			...
			"Task Type:\t\t{6}{5}\n" + 
			"Completion: \t{7}\n" + 
			"Link: \t\t{8}").format(taskName, desc, dateTime, assignedTo, 
									assignedBy, taskType,blank, completion, url)
...
def displayTaskInfo_str(data, title):
    ...
    task = ("Task Name:\t{6}{6}{6}{0}\n" + 
			...
			"Assigned To:\t{6}{3}\n" + 
			"Assigned By:\t{4}\n" + 
			"Task Type:\t\t{6}{5}").format(taskName, desc, dateTime, assignedTo, 
										   assignedBy, taskType, blank)
...
def validDateTime(date_txt):
    ...
    currentDate = dt.datetime.today() + timedelta(hours=-7)  # account for pacific timezone
    taskDate = currentDate.replace(year, months[month], day,
...
async def newTask(ctx, data):
    ...
    if taskNameMsg == True:
	    await ctx.send(embed=errorMessage(taskName = " is a name used for another task"))
    elif dtMsg != " ":
        ...
        # add task to pendingList for approval
	    data = (taskName + "//" + desc + "//" + dateTime + "//" + 
				assignedTo + "//" + assignedBy + "//" + taskType)
	    pendingList.append(data)
        # display task is up for approval
	    await ctx.send(embed=displayTaskInfo_str(data, "Task is Pending Approval"))

# Description: Approve a task to be saved in the Notion database
# @param index: The position of the task within pendingList
# Post-condition: On success, task is removed from pendingList.
#				  Otherwise, an error model will appear in the Discord channel
@bot.command()
async def approvePendingTask(ctx, index):
    if len(pendingList) < 1: # pendingList is empty
        await ctx.send(embed=errorMessage("Pending task list is empty!"))
    elif not index.isdigit(): # index is not an int
        await ctx.send(embed=errorMessage("Specify the index of the task." + 
										  "\nIndex must be an integer"))
    else: # create page in notion
		# split the data
	    data = pendingList[int(index) - 1]
	    dataSplit = data.split("//")
	    taskName = dataSplit[0].strip()
	    desc = dataSplit[1].strip()
	    dateTime = dataSplit[2].strip()
...
# Description: Remove a pending task from pendingList
# @param index: The position of the task within pendingList
# Post-condition: On success, the task will be removed from pendingList
@bot.command()
async def removePendingTask(ctx, index):
    if len(pendingList) < 1: # pendingList is empty
        await ctx.send(embed=errorMessage("Pending task list is empty!"))
    elif not index.isdigit(): # index is not an int
        await ctx.send(embed=errorMessage("Specify the index of the task." + 
	    ...
	    index = int(index) - 1
	    if index > len(pendingList) - 1 or index < 0:
		    await ctx.send(embed=errorMessage("Index is not within the list length"))
	    else: # display on items in the pendingList
		    title = "Task Removed from Pending List!"
		    await ctx.send(embed=displayTaskInfo_str(pendingList.pop(index), title))
			

# Description: List all the pending tasks that have not been approved to be
# 			   added to the Notion database
@bot.command()
async def viewPendingList(ctx):
    # pendingList is empty
    if len(pendingList) < 1:
        await ctx.send("Unapproved task list is empty!")
    # display on items in the pendingList
    else:
	    index = 1
	    await ctx.send("**__Unvalidated Task List__**")
	    for data in pendingList:
			...
			"Assigned By:\t{6}{4}\n" + 
			"Task Type:\t\t{6}{5}").format( taskName, desc, dateTime, assignedTo, 
										  assignedBy, taskType, blank, index)
		    await ctx.send(task)
		    index += 1
...
@bot.command()
async def getNotionTask(ctx, taskName):
...
@bot.command()
async def updateNotionTask(ctx, data):
    # split data into it's values
    dataSplit = data.split("//")
    taskName = dataSplit[0].strip()
	...
	#	{6: Type}, {7: Completion}
	    ...
	    if "name" in field:
		    ...
		    false = ["not done", "not complete", "undone", "undue", "false", "incomplete", "no"]
	    ...
	    if fieldCode == 0:
		    desc = ("field name does not exist. " + 
